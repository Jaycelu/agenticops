  ...
  <div class="workspace-page app-page">
    ...
    <div class="panel-grid">
      <section class="panel-card app-panel">
        <h2>智能体目录</h2>
        <div v-if="catalog.length === 0" class="empty app-empty">暂无目录数据</div>
        <div v-else class="card-list">
          <div v-for="item in catalog" :key="item.agent_type" class="info-card app-subcard">
            <div class="info-head">
              <strong>{{ item.name }}</strong>
              <span class="pill app-pill">{{ item.agent_type }}</span>
            </div>
            <p>{{ item.purpose }}</p>
            <div class="meta-row">
      ...
      <section class="panel-card app-panel">
        <h2>健康度</h2>
        <div v-if="health.length === 0" class="empty app-empty">暂无运行记录</div>
        <div v-else class="card-list">
          <div v-for="item in health" :key="item.agent_type" class="info-card app-subcard">
            <div class="info-head">
              <strong>{{ item.agent_type }}</strong>
              <span class="pill app-pill">{{ item.total_runs }} runs</span>
            </div>
            <div class="stats-row">
    ...
    <section class="panel-card app-panel">
      <h2>最近运行</h2>
      <div v-if="runs.length === 0" class="empty app-empty">暂无运行记录</div>
      <div v-else class="table-like">
        <div v-for="run in runs" :key="run.id" class="table-row app-subcard">
          <div>
