...
- 数据库仅支持 PostgreSQL（`DATABASE_URL` / `AUTOMATION_DATABASE_URL` 必须为 `postgresql://` 或 `postgresql+psycopg2://`）。
- 默认工单模式为本地闭环：`TICKET_MODE=local`。
- 默认安全模式为只读保护：`AUTOMATION_OBSERVE_ONLY=true`。
...
- 启动前请先配置 `backend/.env`（可基于 `deploy/env.example`）：
- `DATABASE_URL`
...
说明：
- 当前仓库已移除测试文件，后端基线以语法检查（`compileall`）和前端构建通过为准。
